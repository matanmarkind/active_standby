...
echo 'sudo RUDRA_RUNNER_HOME=~/rust/Rudra/rudra_runner ~/rust/Rudra/docker-helper/docker-cargo-rudra ~/rust/active_standby && sudo cargo clean'
...
echo 'RUST_BACKTRACE=full cargo test'
RUST_BACKTRACE=full cargo test

echo 'cargo +nightly bench'
cargo +nightly bench

echo 'RUSTFLAGS="--cfg loom" cargo +nightly test --test loom --release'
RUSTFLAGS="--cfg loom" cargo +nightly test --test loom --release

echo 'RUST_BACKTRACE=full RUSTFLAGS="-Zsanitizer=thread -g" cargo +nightly test --lib --release -Z build-std --target x86_64-unknown-linux-gnu # tsan'
...
echo 'RUST_BACKTRACE=full RUSTFLAGS="-Zsanitizer=thread -g" cargo +nightly bench -Z build-std --target x86_64-unknown-linux-gnu # tsan'
...
echo 'cargo clean && cargo +nightly miri test && cargo clean'
