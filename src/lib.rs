...
//! free to check that out. The main differences focus on trying to make the API
//! closer to using an RwLock, to simplify the user experience.
//!
...
//!    an RwLock. These structs can be shared between threads by passing cloning
//!    & sending an Arc<AsLock>. The main difference is that instead of using
//!    AsLock<Vec<T>>, you would use vec::shared::AsLock<T>. This is because
//!    both tables must be updated, so users can't just dereference and mutate
...
//! worrying about :tarving the writer since it will be able to work on the
//! standby table, as opposed to with an RwLock. This means multiple requests
//! can be handled without having to relock the active_table. Similarly you can
//! batch with the Writer without starving the Readers.
//!
...
//!    just implement the mutations for the generated WriteGuard.
