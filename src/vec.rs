...
pub mod vec {
        ...
        T: 'static + Clone,
    {
        pub fn push(&mut self, value: T) {
            let (standby_table, mut enqueue_ops) = self.guard.standby_table_and_enqueue_ops();
            standby_table.push(value.clone());
            enqueue_ops(Box::new(move |table: &mut Vec<T>| table.push(value)));
        }
    }
    ...
    impl<'w, T> WriteGuard<'w, T> {
        pub fn drain<'a, R>(&'a mut self, range: R) -> std::vec::Drain<'_, T>
        where
            R: 'static + Clone + RangeBounds<usize>,
        {
            let range2 = range.clone();
            self.guard.update_tables_individually(
                move |table: &'a mut Vec<T>| table.drain(range),
                move |table: &mut Vec<T>| {
                    table.drain(range2);
                },
            )
        }
